...
export async function POST(req: NextRequest) {
  ...
  for (const file of files) {
    ...
    saved.push({ id: inserted[0].id, filename: recheck.filename, mime_type: recheck.mimeType, size_bytes: bytes.byteLength });
  }
  ...
  if (total > MAX_TOTAL_BYTES) {
    ...
    for (const f of saved) {
      await db()
        .delete(attachments)
        .where(and(eq(attachments.id, f.id), eq(attachments.userId, user.id)));
    }
    return NextResponse.json(
